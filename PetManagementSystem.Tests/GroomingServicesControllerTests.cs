...
namespace PetManagementSystem.Tests
{
    public class GroomingServicesControllerTests
    {
        ...
        {
            // Arrange
            var services = new List<GroomingDTO> { new GroomingDTO { ServiceId = 1, Name = "Wash" } };
            ...
            var response = Assert.IsType<ApiResponse<IEnumerable<GroomingDTO>>>(okResult.Value);
        ...
        {
            // Arrange
            var service = new GroomingDTO { ServiceId = 1, Name = "Wash" };
            ...
            // Assert
            var okResult = Assert.IsType<OkObjectResult>(result.Result);
            var response = Assert.IsType<ApiResponse<GroomingDTO>>(okResult.Value);
            Assert.True(response.Success);
        ...
        {
            // Arrange
            var createDto = new GroomingDTO { Name = "Wash" };
            var createdDto = new GroomingDTO { ServiceId = 1, Name = "Wash" };
            ...
            var response = Assert.IsType<ApiResponse<GroomingDTO>>(createdResult.Value);
            Assert.True(response.Success);
            Assert.Equal(1, response.Data.ServiceId);
        }
        ...
        {
            // Arrange
            var patchDoc = new JsonPatchDocument<GroomingDTO>();
